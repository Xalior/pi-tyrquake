...
# the three kernels and the boot configuration. The game data is not here and
# is not ours to ship — see the README for what to put in tyrquake/id1/ and
# where it legitimately comes from.
CARD_DIR = build/sd-card
card: kernels
	@rm -rf $(CARD_DIR)
	@mkdir -p $(CARD_DIR)/tyrquake/id1
	...
	@echo "  The Raspberry Pi firmware files and the game data are not staged here."
	@echo "  Put Quake's pak0.pak in $(CARD_DIR)/tyrquake/id1/ before writing the card."
